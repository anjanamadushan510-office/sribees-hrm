  ...
  return (
    <div className="space-y-6">
      <header>
        ...
        <p className="mt-1 text-[13px] text-ink-soft">Your time, your balances, and what is coming up.</p>
      </header>
        ...
        <div className="grid gap-6 xl:grid-cols-[minmax(0,2fr)_minmax(0,1fr)]">
            <div className="space-y-6">
              <div className="grid gap-3 sm:grid-cols-3">
                <StatTile label="Today" value={formatDuration(stats.todayMinutes)} hint="Recorded so far" />
                <StatTile
                label="This week"
                value={formatDuration(stats.weekMinutes)}
                hint={`Target ${stats.weekTarget / 60}h`} />
              
                <StatTile label="This month" value={formatDuration(stats.monthMinutes)} hint="Approved and recorded" />
              </div>
            ...
            <div className="space-y-6">
              <Card>
                ...
                action={
                <Link href="/leave" className="text-[13px] font-medium text-brand-700 hover:underline">
                      Request
                    </Link>
                } />
                ...
                <div key={balance.type}>
                      <div className="flex items-baseline justify-between text-[13px]">
                        <span className="text-ink">{LEAVE_TYPE_LABEL[balance.type]}</span>
                        <span className="font-medium text-ink">
                          {balance.remainingDays}
                          <span className="text-ink-soft"> / {balance.entitlementDays} days</span>
                        </span>
                      </div>
                      ...
                      style={{ width: `${balance.pendingDays / Math.max(1, balance.entitlementDays) * 100}%` }} />
                    
                      </div>
                      {balance.pendingDays > 0 ?
                  <p className="mt-1 text-[11px] text-warn-ink">{balance.pendingDays} day(s) awaiting approval</p> :
                  null}
                    </div>
                )}
                </CardBody>
